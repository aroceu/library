<html lang="en">
    
    <head>
        
        <title>aroceu's library (favorites)</title>
        
        <script type="text/javascript">// <![CDATA[
		function ShowHide(divId)
		{
		if(document.getElementById(divId).style.display == 'none')
		{
		document.getElementById(divId).style.display='block';
		}
		else
		{
		document.getElementById(divId).style.display = 'none';
		}
		}
	// ]]></script>
        
        <link rel="stylesheet" href="favorites.css" type="text/css" media="screen" />
        
<?php include('header.php');?>

	<h1>aroceu's favorites</h1>

            <div id="blurb"><p>Here are my favorites &mdash; from the general to the specific &mdash; in a variety of entertainment forms. Though I think of myself as having a wide and diverse range of interests, I am also undeniably drawn to a pattern of preferences, pointed out and exemplified below.</p>
            	<p>There are ships/characters for media that I have dropped here because I want to be thorough. However, my dropping something takes precedence over my favorite ships or characters.</p></div>



            <ul id="favlist">

            <li><a onclick="javascript:ShowHide('themes')" class="h3"><h3>Themes</h3></a>
            <div class="list" id="themes" >
            	
            	<ul class="box">

            		<li>enemies to lovers</li>
            		<li>immature protagonists with hearts of gold</li>
            		<li>main characters / heroes</li>
            		<li>philosophy and the complications of being human</li>
            		<li>time travel and the physical complications of time passing</li>
            		<li>quiet cocky geniuses with a moral code</li>
            		<li>any form of competition, especially when physical (including and especially sports)</li>
            		<li>same-gender characters being trapped together</li>
            		<li>mystery</li>
            		<li>social deception / bottleneck mysteries</li>
            		<li>loyalty to a fault / betrayal and manipulation with sexual tension</li>
            		<li>lazy kids with natural intelligence</li>
            		<li>forbidden romance / forbidden sex</li>
            		<li>red string of fate but why is it you</li>
            		<li>oh shit it's you again and you got hot</li>
            		<li>RPF / finding the romance in everything</li>
            		<li>the fourth wall (and breaking it from the other side)</li>
            		<li>magic with rules</li>
            		<li>satire / parody</li>
            		<li>fairy tales / mythology / folktales / kids' tales</li>
            		<li>comedy-horror</li>
            		<li>boy (genderless) and his dog (genderless) / spongebob and squidward / with the less boisterous one secretly pining for the other</li>
            		<li>romance between two women who have the same male love interest and/or ex-boyfriend</li>
            		<li>emotionally repressed or unexpressive characters who are sticklers for rules</li>
            		<li>female characters who are mean / popular / shallow (bonus points if they are red-headed and/or exceptionally ditzy or intelligent (either works) and/or the eldest child)</li>
            		<li>found family formed from a friendship between an unlikely group of misfits</li> 
            		<li>you're like me but better and what if we kissed</li>
            		<li>the nerve to say or do something for the sake of comedy or entertainment</li>
            		<li>heists that are secretly narratives about found family and friendship</li>
            		<li>class difference ships / "we come from different worlds" romances</li>
            		<li>internalized denial and emotional repression (with consequences, of course)</li>
            		<li>pretty boys who are both flirty and cunning :3</li>
            		<li>redblue ships</li>
            		<li>good boy/brat ships except both parties are both of them</li>
            		<li>matchmaking / genies / cupid / lovebugs / falling in love <i>seemingly</i> under an influence</li>
            	</ul>
            </div></li>

            <li><a onclick="javascript:ShowHide('things')" class="h3"><h3>Things</h3></a>
            <div class="list" id="things" >
            	<img src="https://64.media.tumblr.com/9785fa79612b9bfe5d9d3b266134a906/tumblr_inline_mmpzxtApqz1roozkr.gif" alt="">
            	<ul class="box">
            		<li>dragons</li>
            		<li>oceans</li>
            		<li>elephants</li>
            		<li>sea creatures (dolphins, turtles, sharks, seahorses, plesiosaurus, the loch ness monster)</li>

            		<li>flowers</li>
            		<li>the moon</li>
            		<li>water / ice</li>
            		<li>teleportation</li>
            		<li>vampires</li>
            		<li>werewolves</li>
            		<li>selfcest</li>
            		<li>zombies</li>
            		<li>animation</li>
            		<li>archery / long range attack</li>
            		<li>birds / claws</li>
            		<li>science / math / physics</li>
            		<li>pianos / keyboards</li>
            	</ul>
            </div></li>

            <li><a onclick="javascript:ShowHide('characters')" class="h3"><h3>Characters/People</h3></a>
            <div class="list" id="characters" >

            	<img src="https://64.media.tumblr.com/94e473ca00189239d2c118cd36eed6e2/tumblr_mllhsn9PLU1r7y61oo1_100.gif" alt="">
            	
            	<ul class="box">
            		<li>Aomine Daiki (Kuroko No Basuke)</li>
            		<li>Anakin Skywalker (Star Wars)</li>
            		<li>Angelica Schuyler (Hamilton)</li>
            		<li>Asami Sato (Legend of Korra)</li>
            		<li>Bubbles (Powerpuff Girls)</li>
            		<li>Callie (Splatoon)</li>
            		<li>Carlton Lassiter (Psych)</li>
            		<li>Cho Kyuhyun (Super Junior)</li>
            		<li>Cindy Vortex (Jimmy Neutron)</li>
            		<li>Eren Yeager (SNK)</li>
            		<li>Finn (Star Wars)</li>
            		<li>Gold/Ethan (Pok&eacute;mon - all)</li>
            		<li>Gwaine (Merlin)</li>
            		<li>Gwen Stacy/Spider-Gwen (Marvel - all)</li>
            		<li>Harry Potter (Harry Potter)</li>
            		<li>Haruka Nanase (Free!)</li>
            		<li>Hinata Shouyou (Haikyuu!!)</li>
            		<li>Jack Shepherd (LOST)</li>
            		<li>Jazz Fenton (Danny Phantom)</li>
            		<li>Jiang Cheng (Mo Dao Zu Shi)</li>
            		<li>Kagami Taiga (Kuroko No Basuke)</li>
            		<li>Katara (Avatar: The Last Airbender)</li>
            		<li>Kate Austen (LOST)</li>
            		<li>Kenny McCormick (South Park)</li>
            		<li>Kim Jonghyun (SHINee)</li>
            		<li>Kimiko Tohomiko (Xiaolin Showdown)</li>
            		<li>Kingdra (Pok&eacute;mon)</li>
            		<li>Kino (Kang Hyunggu) (Pentagon)</li>
            		<li>Kunimi Akira (Haikyuu!!)</li>
            		<li>Kurt Wagner/Nightcrawler (X-Men)</li>
            		
            		<li>Lan Wangji (Mo Dao Zu Shi)</li>
            		<li>Lay (Yixing Zhang) (EXO)</li>
            		<li>Lisa (리사) (BLACKPINK)</li>
            		<li>Leigh-Anne Pinnock (Little Mix)</li>
            		<li>Manami Sangaku (Yowapeda)</li>
            		<li>Mayuzumi Chihiro (Kuroko No Basuke)</li>
            		<li>Miles Straume (LOST)</li>
            		<li>Neji Hyuuga (Naruto)</li>
            		<li>Niall Horan (One Direction)</li>
            		<li>Nick Jonas (Jonas Brothers)</li>
            		<li>Nie Mingjue (Mo Dao Zu Shi)</li>
            		<li>Peter Parker/Spider-man (Marvel - all)</li>
            		<li>Princess Morbucks (Powerpuff Girls)</li>
            		<li>Raymond Holt (Brooklyn 99)</li>
            		<li>Regulus Black (Harry Potter)</li>
            		<li>Rin Hoshizora (Love Live!)</li>
            		<li>Rocket Raccoon (Marvel - Guardians of the Galaxy)</li>
            		<li>Rosalina (Super Mario - series)</li>
            		<li>Rose/Huntsgirl (American Dragon: Jake Long)</li>
            		<li>Ryan Ross (Panic! at the Disco)</li>
            		<li>Sansa Stark (Game of Thrones)</li>
            		<li>Scipio Fortunato (The Thief Lord)</li>
            		<li>Scott McCall (Teen Wolf)</li>
            		<li>Shiver (Splatoon)</li>
            		<li>Sirius Black (Harry Potter)</li>
            		<li>Sulli (Choi Jinri) (f(x))</li>
            		<li>Tahani Al-Jamil (The Good Place)</li>
            		<li>Tanaka Ryuunosuke (Haikyuu!!)</li>
            		<li>Tia (Animal Crossing)</li>
            		<li>Treecko / Grovyle / Sceptile (Pok&eacute;mon)</li>
            		<li>U-Kwon (Kim Yukwon) (Block B)</li>
            		<li>Vader Eloha (OFF)</li>
            		<li>Veronica Lodge (Riverdale)</li>
            		<li>Villager (Mario Kart)</li>
            		<li>Violet Baudelaire (A Series of Unfortunate Events)</li>
            		<li>Wei Wuxian (Mo Dao Zu Shi)</li>
            		<li>White/Hilda (Pok&eacute;mon - all)</li>
            		<li>Yang Hongseok (Pentagon)</li>
            		<li>Yellow (Pok&eacute;mon Adventures)</li>
            		<li>Zero Suit Samus (Super Smash Bros.)</li>
            	</ul>
            </div></li>

            <li><a onclick="javascript:ShowHide('ships')" class="h3"><h3>Ships</h3></a>
            <div class="list" id="ships" >

            	<img src="https://64.media.tumblr.com/c749c88e579e9039d312dc0b783d7fa1/tumblr_inline_mkn8j0ALNl1roozkr.gif" alt="">
            	
            	<ul class="ships">
            		<li class="yuri">Agent 3/Agent 8 (Splatoon)</li>
            		<li class="yaoi">Albus Severus/Scorpius (Harry Potter)</li>
            		<li class="yuri">Amy/Rosa (Brooklyn 99)</li>
            		<li class="yuri">Angelica/Susie (Rugrats/All Grown Up)</li>
            		<li class="poly">Annie/Troy/Abed (Community)</li>
            		<li class="yaoi">Aomine/Kagami (Kuroko No Basuke)</li>
            		<li class="yaoi">Barry/Oliver (Arrowverse)</li>
            		<li class="het">Beast Boy/Terra (Teen Titans)</li>
            		<li class="yuri">Beca/Chloe (Pitch Perfect)</li>
            		<li class="yuri">Blossom/Princess Morbucks (Powerpuff Girls)</li>
            		<li class="yaoi">Butters/Kenny (South Park)</li>
            		<li class="yuri">Christine/Meg (Phantom of the Opera)</li>
            		<li class="yaoi">Draco/Harry (Harry Potter)</li>
            		<li class="yaoi">Eduardo/Mark (The Social Network)</li>
            		<li class="yaoi">Eren/Jean (SNK)</li>
            		<li class="yaoi">Gabe Saporta/William Beckett (Fueled By Ramen)</li>
            		<li class="yuri">Gabriella/Sharpay (High School Musical)</li>
            		<li class="yuri">Hollywood/Katelin (Go Figure)</li>
            		<li class="yaoi">Hongseok/Kino (Pentagon)</li>
            		<li class="yuri">Honoka/Kotori/Umi (Love Live!)</li>
            		<li class="het">Jake/Rose (American Dragon: Jake Long)</li>
            		<li class="yuri">Karin/Blue (Black Mirror: Hated in the Nation)</li>
            		<li class="yaoi">Kageyama/Hinata (Haikyuu!!)</li>
            		<li class="yuri">Kate/Juliet (LOST)</li>
            		<li class="yaoi">Kindaichi/Kunimi (Haikyuu!!)</li>
            		<li class="yuri">Korra/Asami (Legend of Korra)</li>
            		<li class="yuri">Krista/Nadine (The Edge of Seventeen)</li>
            		<li class="yaoi">Kuroko/Mayuzumi (Kuroko No Basuke)</li>
            		<li class="het">Kurt/Kitty (X-Men: Evolution)</li>
            		<li class="yaoi">Kyuhyun/Yesung (Super Junior)</li>
            		<li class="yaoi">Lan Wangji/Wei Wuxian (Mo Dao Zu Shi)</li>
            		<li class="yaoi">Light/L (Death Note) (I also like adding Misa in every so often)</li>
            		<li class="yaoi">Link/Sidon (Legend of Zelda)</li>
            		<li class="yuri">Mabel/Pacifica (Gravity Falls)</li>
            		<li class="yaoi">Makoto/Haru (Free!)</li>
            		<li class="poly">Maria/Manolo/Joaquin (Book of Life)</li>
            		<li class="het">Mark/Courtney (Pendragon)</li>
            		<li class="yuri">Mary-Jane/Gwen (Marvel)</li>
            		<li class="yaoi">Merlin/Arthur (Merlin)</li>
            		<li class="yaoi">Oikawa/Suga (Haikyuu!!)</li>
            		<li class="yaoi">Pat/Pran (Bad Buddy)</li>
            		<li class="yuri">Pearl/Marina (Splatoon)</li>
            		<li class="yaoi">Poe/Finn (Star Wars)</li>
            		<li class="yaoi">Raimundo/Omi (Xiaolin Showdown)</li>
            		<li class="yaoi">Red/Gold (Pok&eacute;mon)</li>
            		<li class="het">Ron/Hermione (Harry Potter)</li>
            		<li class="yaoi">Ryan/Brendon (Panic! at the Disco)</li>
            		<li class="yaoi">Sasuke/Naruto (Naruto)</li>
            		<li class="poly">Shawn/Gus/Juliet (Psych)</li>
            		<li class="yaoi">Stan/Kyle (South Park)</li>
            		<li class="yuri">Tahani/Eleanor (The Good Place)</li>
            		<li class="yaoi">Wonil/Woosang (No Breathing)</li>
            		<li class="yaoi">Zayn/Niall (One Direction)</li>
            	</ul>
            </div></li>

            

        </ul>
	        
	        <?php include('footer.php');?>
	        